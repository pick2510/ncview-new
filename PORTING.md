...
- **Side-by-side parity checklist against the Xaw build**: partial, not exhaustive. Verified via Xvfb screenshots as each feature was implemented (M3's main window/colormap cycling/colorbar, M4's range/options/dimset/print/info/dataedit/plot dialogs, M5's PNG dump matching the on-screen render pixel-for-pixel). Not separately re-verified in one final side-by-side pass against the original Xaw binary: blowup/transform mode combinations, overlay rendering, and `.ncviewrc` round-tripping through a real save/restart cycle. Left as follow-up QA rather than done here.
