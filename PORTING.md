...
- ✅ **Data-edit grid** (`x_dataedit`/`in_set_edit_place`): an `Fl_Table` (`ui/src/interface_fltk.cc`'s `DataEditTable`) backed directly by the `char**` cell-text buffer upstream hands over; click a cell to `fl_input()` a new value, which calls `view_change_dat()` and rewrites the cell in place. "Dump Data" calls `view_data_edit_dump()`. `in_set_edit_place()` drives `Fl_Table::set_selection`/`row_position`/`col_position` to highlight/scroll to a cell -- wired to the image view's middle-button press/drag (`set_dataedit_place()`), matching upstream's `Btn2Up`/`Btn2Motion` translation that this port had not yet wired up.
  - **Post-v0.2.3**: "Dump Data" (`view_data_edit_dump()`) and the XY plot window's "Dump" (`PlotWindow::dumpCallback`) both used to prompt for an output filename with a plain text-input dialog (`in_dialog()`'s free-text mode, `fl_input()`) instead of a real file browser. Replaced with `Fl_Native_File_Chooser` in `BROWSE_SAVE_FILE` mode -- a new seam function, `in_choose_save_file()`, for the core-side one (`view_data_edit_dump()` can't reference FLTK directly); the plot-window one is pure UI code already, so it calls `Fl_Native_File_Chooser` straight from `ui/src/plot_window.cc`. `in_dialog()`'s `ret_string`/`ret_string_size` parameters, which existed solely for this one caller, were dropped -- everything else that calls `in_dialog()` only ever used its message/confirm mode.
...
- ✅ **Printing** (`do_print.c`): stayed as core logic essentially unchanged since M1 through v0.2.3 (`core/src/do_print.cc` hand-wrote PostScript and shelled out to `lpr` exactly like upstream's "Printer" device -- there was no native print dialog to replace, so `Fl_Printer` was considered new scope, not parity, and wasn't added). **Post-v0.2.3**: replaced with `Fl_Printer` (main window) and `Fl_Printer::print_widget()` (XY plot window) -- the PostScript generator and `lpr` shell-out are gone, and printing now works on Windows for the first time (`mkstemp`/`lpr` never existed there). `do_print.cc` still gathers the page metadata (title, axis labels, extra-info block, ID stamp) via the same `fi_*` calls as before; `ui/src/interface_fltk.cc`'s new `in_print()` draws it via `fl_draw()`/`fl_rect()` instead of `fprintf()`-ing PostScript operators. The old Printer Options dialog's Device/output-filename fields are gone (the native print dialog covers that); its margin/font/include-toggle fields survive, trimmed, as a "Print Layout" dialog.
...
- **Button-bar parity pass**: added an `NCVIEW_TEST_BUTTON=<name>` test hook (`ui/src/interface_fltk.cc`, alongside the existing `NCVIEW_TEST_DIALOG`) that calls `in_button_pressed()` directly -- the exact path `MainWindow::buttonCallback` uses for a real click -- so every button can be regression-checked under Xvfb without a real mouse. Screenshotted blowup, blowup-type (bilinear/replicate), transform, invert-colormap, and invert-physical: all behave correctly.
  ...
  - **Found and fixed post-v0.2.3, a real gap missed by M3/M4**: upstream's `do_buttons.c` gives the transport buttons and the Range button `Modifier` semantics -- Ctrl+click on Rewind/Backwards/Forward/Fastforward jumps by a percentage of the frame count instead of one frame; right-click on Range sets the color range from just the current frame instead of opening the dialog (`data/ncview.1`'s "MODIFIERS" section). `MainWindow::buttonCallback` always passed `Modifier::M1` regardless of mouse button or Ctrl state, even though `do_buttons.cc` still carries every `M2`/`M3`/`M4` branch verbatim -- both actions were simply unreachable. Fixed: `buttonCallback` now checks `Fl::event_ctrl()` for the four transport buttons (still real toolbar buttons, so the gesture translates directly); Range's frame-only variant got a second, explicit menu entry ("View/Range (Current Frame)") since it's a menu item now and there's no right-click gesture on one. Reverse Transform/colormap cycling (also right-click, upstream) were not restored -- both are a menu item/combobox now, forward-only cycling already reaches every state, and the colormap combobox selects directly rather than cycling at all, so there's nothing for "reverse" to mean there.
